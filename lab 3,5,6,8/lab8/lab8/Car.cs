...
namespace lab8
{
    class Car : IVehicle, IComparable
    {
        ...
        public delegate void ReachDestinationHandler(string message);
        protected event ReachDestinationHandler ReachDestination;
        protected delegate bool ValidateString(string str);
        ...
        public Car(uint weight, uint age, uint fuelCapacity, uint currentFuel,uint fuelUsage, string brand, ReachDestinationHandler destHandler)
        { 
            id = amount + 1; 
            amount++; 
            Weight = weight; 
        ...
        { 
            ...
            { 
                Console.WriteLine("Too big distance, not enought fuel. Go to gas station. Type yes to go to gas station");
            ...
            ReachDestination("You reached your destination");
        ...
        { 
            ...
            if (distance*FuelUsage>CurrentFuel)
            {
                ValidateString isYes = str => str == "yes";
                Console.WriteLine("Too big distance, not enought fuel. Go to gas station. Type \"yes\" to go to gas station");
                var key = Console.ReadLine().ToLower(); 
                if (isYes(key)) 
                { 
